...
namespace ZaroDev.Utils.Runtime.Common
{
    ...
    public class GOPool<T> : IPool<T> where T : MonoBehaviour, IPoolable<T>
    {
        public GOPool(GameObject pooledObject, int numToSpawn = 0)
        {
            this.prefab = pooledObject;
            Spawn(numToSpawn);
        }

        public GOPool(GameObject pooledObject, Action<T> pullObject, Action<T> pushObject, int numToSpawn = 0)
        {
            this.prefab = pooledObject;
